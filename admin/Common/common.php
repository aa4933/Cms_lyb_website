...
// 管理员操作日志
function admin_log($log_op,$log_obj,$log_desc)
{
	$admin_log = D('op_log');

	$data = array ( 'USER_NAME' => $_SESSION['admin_info']['user_name'], 'OP'=> $log_op, 'OP_TIME' => date('Y-m-d H:i:s'), 'OP_OBJECT'	 => $log_obj,'OP_DESC'	 => $log_desc);
	$insertId=$admin_log->add($data);
}
